...
describe("BuildingCard — expand/collapse", () => {
  it("el panel expandido no es visible inicialmente", async () => {
    (fetchEspaciosPorEdificio as ReturnType<typeof vi.fn>).mockResolvedValue([]);
    renderCard();
    // JSDOM no computa CSS, así que no podemos usar toBeVisible().
    // Verificamos que el contenedor colapsable tenga la clase Tailwind max-h-0.
    const descripcion = screen.queryByText(
      "Edificio principal de la facultad de ciencias sociales."
    );
    const colapsable = descripcion?.closest(".overflow-y-auto");
    expect(colapsable).toHaveClass("max-h-0");
  });

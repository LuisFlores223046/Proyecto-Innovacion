...
describe("EspacioCard — contenido básico", () => {
  it("muestra el nombre del espacio", () => {
    ...
    expect(screen.getByText(/Laboratorio de Cómputo 101/, { selector: "h3" })).toBeInTheDocument();
  });
...
describe("EspacioCard — categoría", () => {
  it("muestra el nombre de la categoría cuando existe", () => {
    renderCard(espacioConCoordenadas);
    // La categoría aparece en un <span>; el h3 también contiene "Laboratorio" así que
    // buscamos específicamente el span que tiene ese texto.
    expect(screen.getByText(/Laboratorio/, { selector: "span" })).toBeInTheDocument();
  });
...
describe("EspacioCard — navegación", () => {
  ...
  it("no navega al hacer click en un espacio sin coordenadas", () => {
    ...
    // El h3 contiene "📍 Aula sin ubicación", usamos regex para encontrarlo.

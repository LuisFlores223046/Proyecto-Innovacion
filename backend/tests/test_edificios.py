...
class TestListarPisos:
    ...
    async def test_listar_pisos_de_edificio(
        ...
        assert response.json()[0]["numero"] == "PB"
...
class TestCrearPiso:
    ...
    async def test_crear_piso_exitoso(
        ...
        assert response.status_code == 201
        assert response.json()["numero"] == "P1"
    ...
    async def test_crear_piso_duplicado_retorna_409(
        ...
        response = await client.post(
            "/api/v1/pisos",
            json={"edificio_id": edificio_prueba.id, "numero": "PB"},
            headers=auth_headers,
        )
        assert response.status_code == 409
    ...
    async def test_crear_piso_sin_token_retorna_403(
        ...
        response = await client.post(
            "/api/v1/pisos",
            json={"edificio_id": edificio_prueba.id, "numero": "P9"},
        )
        assert response.status_code in (401, 403)
